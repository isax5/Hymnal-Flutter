...
class DraggablePlayerState extends State<DraggablePlayer> with TickerProviderStateMixin {
  ...
  void _onAudioChanged() {
    if (_audioService.isDismissing &&
        !_slideOutController.isAnimating &&
        _slideOutController.value == 0) {
      _controller.reverse().then((_) {
        if (mounted) {
          _slideOutController.forward().then((_) {
            if (mounted) _audioService.finalizeDismiss();
          });
        }
      });
    }
  ...
  void dispose() {
    ...
    if (_audioService.isDismissing) _audioService.finalizeDismiss();
    _controller.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      animation: Listenable.merge([_controller, _slideOutController, _audioService]),
      builder: (context, child) {
        if (_audioService.currentHymn == null) {
          return const SizedBox.shrink();
        }

        final expansionValue = _controller.value;
        final slideOutValue = _slideOutController.value;
        ...
        return Positioned(
          ...
          child: Transform.translate(
            offset: Offset(0, effectiveMinHeight * slideOutValue),
